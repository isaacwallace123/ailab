...
variable "node_name" {
  description = "Proxmox node hosting the B580 AI worker."
  type        = string
  default     = "cyberlab"

  validation {
    condition     = var.node_name == "cyberlab"
    error_message = "ai-node-02 must initially target the cyberlab Proxmox node."
  }
}
...
variable "vm_id" {
  description = "Reserved VM ID for ai-node-02."
  type        = number
  default     = 9601

  validation {
    condition     = var.vm_id == 9601
    error_message = "The reviewed VM ID for ai-node-02 is 9601."
  }
}
...
variable "template_vm_id" {
  ...
  validation {
    ...
    error_message = "The B580 AI worker must use the reviewed generic template 9000."
  }
}
...
variable "gpu_mapping" {
  description = "Existing Proxmox PCI resource mapping for the Intel Arc B580."
  type        = string
  default     = "ailab-intel-arc-b580"

  validation {
    condition     = var.gpu_mapping == "ailab-intel-arc-b580"
    error_message = "The second GPU mapping must be ailab-intel-arc-b580."
  }
}

variable "mac_address" {
  description = "Stable MAC address for the ai-node-02 DHCP reservation."
  type        = string
  default     = "BC:24:11:09:A0:46"
